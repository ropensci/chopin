...
```{r test-clip-extent-vector, eval = FALSE}
...
#' @title Quick wrapper for parallel processing
#' @param fun function name without parenthesis.
#' @param at SpatVector/sf. Location/area where
#'  covariates will be calculated.
#' @param from SpatRaster/SpatVector/sf. From which outcomes are
#' calculated.
#' @param grids SpatVector. Grids each of which will get
#'  split work demand. Should have an identifier field
#'  named "CGRIDID".
#' @param grid_ref character(1). internally passed to
#'  \code{distribute_*} function. If not NULL, grid mode is
#' activated.
#' @param ... passed to \code{fun}. Should follow the order of
...
dispatcher <-
  function(fun,
           ...
           ...) {
    # parsed_args <- list(...)
    if (!methods::is(at, "SpatVector")) {
      at <- switch_packbound(at)
    }
    if (check_packbound(from) != "terra") {
      from <- switch_packbound(from)
    }
    # detect class
    # args_class_detected <- lapply(parsed_args, \(x) class(x)[1])
    # crop or partially read input

    grid_now <- grids[grids$CGRIDID == grid_ref, ]
    at_sub <- terra::crop(at, grid_now)
    from_sub <- terra::crop(from, grid_now)
    fun(at_sub, from_sub, ...)
  }
...
```{r test-runhierarchy, eval = FALSE}
  ...
  nccompreg <-
    get_computational_regions(
      ...
      \(x) {
        ...
        run_result <- tryCatch({
          res <- extract_with_buffer(ncpnts, ncelev, 5e3L, "pid", grid_ref = x)
          cat(sprintf("Your input function was 
          successfully run at CGRIDID: %s\n",
