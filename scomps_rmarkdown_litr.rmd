...
kk <- extract_with_buffer_kernel(ncp, ncelev, 3000L, "pid", 90L, kernel = "epanechnikov", bandwidth = 4000L)
head(kk)
...
```{r test-within-reference, eval = FALSE}
...
```{r test-detect-class, eval = FALSE}
...
```{r test-generic-function, eval = FALSE}
...
```{r test-multirasters, eval = FALSE}
...
if (dir.exists("../figure")) {
  ...
  file.copy(from = "../tools/vignettes-sources/figure", to = "./vignettes/figure", recursive = TRUE)
  # unlink("../figure", recursive = TRUE)
}
pkgbuild::build(path = ".")
