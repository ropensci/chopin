...
usethis::create_package(
  ...
  fields = list(
    ...
    Depends = "R (>= 4.1)",
    URL = "https://github.com/kyle-messier/chopin",
    BugReports = "https://github.com/kyle-messier/chopin/issues"
  )
)
...
par_make_gridset <-
  function(
    ...
    if (!is.numeric(padding)) {
      message("padding should be numeric. We try converting padding to numeric...\n")
...
```{r grid-balanced, send_to = "R/gridding.R"}
#' Generate grids based on balanced clustering
...
#' @param points_in `sf` or `SpatVector` object. Target points of computation.
#' @param num_cluster integer(1). The number of clusters.
#' @returns `sf` or `SpatVector` object with rectangular polygons.
#' @examples
#' library(terra)
#' library(anticlust)
#' data(ncpoints, package = "chopin")
...
#' @export
par_grid_balanced <- function(
  points_in = NULL,
  num_cluster = NULL
) {
  if (!is.numeric(num_cluster)) {
    stop("num_cluster should be numeric.\n")
  }
  if (num_cluster < 2) {
    stop("num_cluster should be greater than 1.\n")
  }
  if (dep_check(points_in) == "sf") {
    points_in <- terra::vect(points_in)
  }
  #dissim <- as.matrix(terra::distance(points_in))
  dissim <- dist(terra::crds(points_in), method = "minkowski", p = 4)
  cl <- anticlust::balanced_clustering(dissim, K = num_cluster)
